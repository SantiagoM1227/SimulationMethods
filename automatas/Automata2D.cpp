...
double LatticeGas::Varianza(void){
  ...
  //Calcular N
  for(N=0,ix=0, iy=0;ix<Lx, iy<Ly;ix++, iy++)
    N+=rho(ix,iy);
  //Calcular Xprom Yprom
  for(Xprom=0, Yprom = 0, ix=0, iy=0 ;ix<Lx,iy<Ly;ix++,iy++)
    Xprom+=ix*rho(ix,iy);
    Yprom+=iy*rho(ix,iy);
  Xprom/=N; Yprom/=N;
  //Calcular Sigma2
  for(Sigma2=0,ix=0,iy=0;ix<Lx,iy<Ly;ix++,iy++)
